...
Return JSON in this exact format:
{
  ...
  private async convertToExecutions(
    ...
    for (const [_index, toolCall] of toolCalls.entries()) {
      const action = request.availableActions.find(a => a.name === toolCall.tool);
      if (!action) {
        logger.warn('Action not found in available actions', { actionName: toolCall.tool });
        continue;
      }
      ...
      const execution: ToolCallExecution = {
        ...
        sessionId: request.sessionId,
        actionName: toolCall.tool,
        parameters: toolCall.parameters,
        reasoning: 'Generated by AI',
        confidence: 0.8,
        riskLevel: action.sideEffecting === 'destructive' ? 'high' : 'low',
        requiresConfirmation: this.shouldRequireConfirmation(action, action.sideEffecting === 'destructive' ? 'high' : 'low'),
        retryCount: 0,
